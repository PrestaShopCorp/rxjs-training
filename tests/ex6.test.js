...
describe("Exercice6", () => {
    ...
    it('Return an observable', () => {
        expect(this.exercice.estimateWithRetry(['safe-address'])).toBeInstanceOf(Observable);
    });

    it("Emit a value for each of the address to estimate", (done) => {

        this.exercice.estimateWithRetry(['safe-address-1', 'safe-address-2', 'safe-address-3']).pipe(toArray())
            .subscribe(
                n => {
    ...
    it("Retry on API error", (done) => {
        ...
        this.exercice.estimateWithRetry(['address-1', 'address-2']).pipe(toArray())
            .subscribe(
                n => {
    ...
    it("Call the API 4 times per address max", (done) => {
        ...
        this.exercice.estimateWithRetry(['buggy-address']).pipe(toArray())
            .subscribe(
                n => {
    ...
    it("Adresses impossible to estimate due to a high amount of errors must be ignored", (done) => {
        ...
        this.exercice.estimateWithRetry(['10 rue du test', 'buggy-address', '11 rue du test', ]).pipe(toArray())
            .subscribe(
                n => {
                    expect(n.length).toEqual(2);
                    expect(n).toEqual([
