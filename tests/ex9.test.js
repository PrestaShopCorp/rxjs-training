...
describe("Exercice9", () => {
  it("Return an observable", () => {
    ...
    expect(ex.buyTicketWithRetry("Pouet against the machine")).toBeInstanceOf(
      Observable
    );
  });
  ...
  it("Return an error if the API respond with error Invalid concertId", (done) => {
    ...
    ex.buyTicketWithRetry("invalid-concert").subscribe(
      null,
      (err) => {
      ...
      () => {
          ...
          new Error(
            "Observable must return an error when API respond with an error sInvalid concertId"
          )
        );
      }
    );
    ...
    ex.buyTicketWithRetry("Chantal Goya")
      ...
      .subscribe(
        (n) => {
          ...
          expect(n[0]).toEqual({
            ticketId: "662db134-801c-42d3-842f-2e649e89c151",
            concert: "Britney Spears tour 2020",
            status: "reserved",
          });
  ...
  it("Timeout after 3s", (done) => {
    ...
    ex.buyTicketWithRetry("2 Unlimited - No limit tour").subscribe(
      null,
      (err) => {
